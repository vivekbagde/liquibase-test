pipeline {
    ...
    stages {
        stage('info') {
            ...
            steps {
                sh 'bash $JENKINS_HOME/workspace/LiquibaseDeploymentDemo-Pipeline/liquibase generateChangelog --url=jdbc:postgresql://devopstestdb.chzkxi0lcrpk.ap-south-1.rds.amazonaws.com:5432/example --driver=org.postgresql.Driver --classpath=$JENKINS_HOME/workspace/LiquibaseDeploymentDemo-Pipeline/liquibase/postgresql-42.2.23.jar --changeLogFile=$JENKINS_HOME/workspace/LiquibaseDeploymentDemo-Pipeline/databaseChangeLog.sql --username=$SECRET_VALUE_USR --password=$SECRET_VALUE_PSW'
